...
void get_test_params(TestParams& params)
{
    ...
    pp.get("plot_int",params.plot_int);
}

void main_main ()
{

    amrex::Print() << " ooo Running MD benchmark \n";
    ...
    // Create a vector of geometries with one entry
    amrex::Vector<amrex::Geometry> geom_vec(1);
    geom_vec[0] = geom;
    ...
    amrex::Print() << " Number of Steps " << params.nsteps << "\n";
    ...
    for (int step = 0; step < params.nsteps; ++step) {
        ...
        // plotting 
        if ((params.plot_int > 0) && (step % params.plot_int == 0)) {
           //  WritePlotFile (pc, geom_vec, step);
            WriteParticlesVTK(pc, step);
        }
